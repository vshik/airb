...
class DB3_GTR4_FLOW(IData):
    ...
    def _parse(self, path):
        """
        PARSE REF FILE FROM A NETWORK LOCATION RETURN A DATAFRAME WITh one row corresponding to one test reference.

        :type self: DB3_GTR4_FLOW
        ...
        # gtr data
        flow_df = pd.read_excel(path, sheetname="DB3 GTR4", parse_cols='I:HK', skiprows=1, header=[0,1,2])        # using 'parse_cols' instead of 'usecols' because of older pandas version     #Starting from column 'I', otherwise C17-18 will get dropped
        flow_df.columns = flow_df.columns.map('_'.join)

        # gtrdef data
        flow_gtrdef_df = pd.read_excel(path, sheetname="DB3 GTR4", parse_cols='J:HK', header=None)        # using 'parse_cols' instead of 'usecols' because of older pandas version
...
if __name__ == "__main__":
    ...
    GTR4FLOW = DB3_GTR4_FLOW()

    # Test 1
    GTR4FLOW._parse(reffilepath)
    GTR4FLOW.headdf()
    testfile = GTR4FLOW.export2Excel("test1DB3_FLOW.xlsx", export_folderpath)
